...
class BotApiTransport:
    ...
    def _call(self, method: str, payload: Optional[dict] = None) -> dict:
        ...
        try:
            ...
            log.warning("bot_api %s failed", method, exc_info=True)
            return {}
        if not body.get("ok"):
    ...
    def do_reply(self, peer: Any, msg_id: int, text: str) -> bool:
        payload: dict = {"chat_id": peer, "text": str(text)[:4096]}
        if msg_id and int(msg_id) > 0:
            payload["reply_parameters"] = {"message_id": int(msg_id),
                                           "allow_sending_without_reply": False}
        return bool(self._call("sendMessage", payload))
